...
$mac="9932423333";     //registered on intranet
#print "<pre>". print_r($radiusData,true) ."</pre>"; 

$form = 'checkPhone';     //default page in case mac doesn't exist
if($mac) {
        $radiusData = preg_split("/\,/", radiusAPI(xmlrpc_encode_request("CHK",$mac)));
	$radiusData = $radiusData[0];
 
        #If mac exists Radius
        if ($radiusData) {  
                $form = 'login';
        }
        #If phone exists on GuestBook but mac doesn't exist Radius, add mac to radius
        else if ($phone){
                $form = 'addRadius';
        }
        #if phone variable empty, ask user for their phone number first
        else {  
                $form = 'checkPhone';
        }
}
else{
        $message = "Could not retrieve mac address";
...
HTML;
        ...
        <div id="center">
                ...
                <h1>
                        Welcome!
                        <p id="subheader">Please enter your phone number below</p>
                </h1>
                <form  action="goto.php" method="post" name="regForm" onsubmit="return isEmpty(this.phone.value);">
                        <input type="hidden" name="form" value="$form"/>
                        <input type="hidden" name="username" value="$mac"/>
                        <input type="hidden" name="password" value="$mac"/>
